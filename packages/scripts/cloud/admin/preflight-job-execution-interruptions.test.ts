...
describe("job interruption catalog preflight", () => {
  ...
  test("guards both deploy restart and every systemd worker activation", async () => {
    ...
    expect(workflow).toContain(
      "- 'packages/scripts/cloud/admin/error-preserving-cleanup.ts'",
    );
    ...
    const schemaPreflight = service.indexOf(
      "ExecStartPre=/usr/bin/env bun --conditions=eliza-source /opt/eliza/packages/scripts/cloud/admin/preflight-job-execution-interruptions.ts",
    );
